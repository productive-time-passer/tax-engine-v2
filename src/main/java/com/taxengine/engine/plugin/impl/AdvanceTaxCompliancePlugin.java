...
public class AdvanceTaxCompliancePlugin implements Plugin {

    @Override
    public String pluginId() {
        return "AdvanceTaxCompliancePlugin";
    }
    ...
    public Set<String> dependencies() {
        return Set.of("ItrFilingCompliancePlugin");
    }
    ...
    public PluginResult evaluate(TaxContext context, TaxComputationState state, List<RuleConfig> rules) {
        ...
        for (RuleConfig rule : rules) {
            BigDecimal liability = state.getTotalByPrimitive(PrimitiveType.TAX_RATE)
                    .subtract(state.getTotalByPrimitive(PrimitiveType.TAX_CREDIT)).max(BigDecimal.ZERO);
            BigDecimal threshold = RuleParamReader.decimal(rule.parameters(), "threshold", new BigDecimal("10000"));
            boolean required = liability.compareTo(threshold) > 0;
            BigDecimal flag = required ? BigDecimal.ONE : BigDecimal.ZERO;
            computations.add(PluginComputationFactory.singleImpact(pluginId(), primitive(), rule,
                    Map.of("liability", liability), "COMPLIANCE", "ADVANCE_TAX_REQUIRED", flag, flag, flag,
                    TaxImpactType.COMPLIANCE, "Advance tax compliance check", context.taxpayer().taxpayerId()));
        }
