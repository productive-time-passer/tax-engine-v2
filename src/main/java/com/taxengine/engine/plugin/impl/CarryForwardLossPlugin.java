...
public class CarryForwardLossPlugin implements Plugin {

    @Override
    public String pluginId() {
        return "CarryForwardLossPlugin";
    }
    ...
    public Set<String> dependencies() {
        return Set.of("CapitalLossSetoffPlugin");
    }
    ...
    public PluginResult evaluate(TaxContext context, TaxComputationState state, List<RuleConfig> rules) {
        ...
        for (RuleConfig rule : rules) {
            BigDecimal total = context.factIndex().totalByType(FactType.CARRY_FORWARD);
            ...
            computations.add(PluginComputationFactory.singleImpact(pluginId(), primitive(), rule,
                    Map.of("total", total), "ADJUSTMENT", "CARRY_FORWARD", allowed, total, allowed,
