...
public class InsuranceDeductionPlugin implements Plugin {

    @Override
    public String pluginId() {
        return "InsuranceDeductionPlugin";
    }
    ...
    public Set<String> dependencies() {
        return Set.of("InvestmentDeductionPlugin");
    }
    ...
    public PluginResult evaluate(TaxContext context, TaxComputationState state, List<RuleConfig> rules) {
        ...
        for (RuleConfig rule : rules) {
            BigDecimal total = context.factIndex().totalByType(FactType.INSURANCE);
            ...
            computations.add(PluginComputationFactory.singleImpact(pluginId(), primitive(), rule,
                    Map.of("total", total), "DEDUCTION", "80D", allowed, total, allowed,
